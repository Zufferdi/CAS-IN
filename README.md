...
| Section | Contenu |
|---|---|
| 💊 **Pilule bleue** — Fiches | 109 fiches de révision structurées par catégorie (FS, Windows, crypto, réseau, droit, plateformes, acquisition) |
| 💊 **Pilule verte** — TP | 27 catégories d'exercices : FAT, NTFS, exFAT, EXT, HFS+, endianness, magic bytes, hashes, droit pénal, email, réseau, IR, Registry, Prefetch, LNK, etc. |
| 💊 **Pilule orange** — Scènes | 136 scénarios DFIR immersifs avec choix multiples, conséquences procédurales, références légales (Art. 141 CPP, ACPO, NIST) |
| 💊 **Pilule rouge** — Quiz | 1906 questions gamifiées (XP, rangs, streaks, défi quotidien, mode survie, SM2 spaced repetition) |
...
> dette technique), voir **[docs/ARCHITECTURE.md](docs/ARCHITECTURE.md)**.

```
CAS-IN/
├── index.html              # Landing (Matrix rain, drawer profil, raccourcis B/V/O/R)
├── quiz.html               # 1906 questions gamifiées
...
├── docs/                   # ⭐ Documentation dev (déplacée v2.11)
│   ├── ARCHITECTURE.md     # Documentation des couches & ordre de chargement
│   └── CHANGELOG.md        # Keep-a-Changelog
│
├── manifest.json           # Source de vérité : 109 fiches × 7 catégories
├── pwa.manifest.json       # Manifest PWA (W3C)
├── counts.json             # Auto-généré : nombres affichés partout
├── questions.json          # 1906 questions
│
├── scenes/                 # 136 scènes DFIR (lazy-load v3.0)
